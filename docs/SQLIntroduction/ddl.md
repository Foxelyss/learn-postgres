...
Идентификатор - название чего-либо в БД 

Команда - некоторое ключевое слово

Оператор - ключевое слово которое расширяет функционал команды
...
Главные правила :
1. Максимальная длина 128 символов
2. ИНДФ не начинается с числа
3. ИНДФ не содержит пробелов(а если содержит, то пишется только в двойных кавычках)
...
Нюансы:
1. Все новые столбцы будут добавлены в конец таблицы
2. Можно добавлять несколько столбцов за 1 раз через запятую
3. Если есть хотя бы одна строка данных, то добавить ограничение not null не получится
...
Оператор команды есть только один - cascade, для удаление связей между таблицами(Foreign Key)
